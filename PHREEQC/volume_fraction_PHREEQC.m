function [P,Tmat,Smat,Vi_V, Vb_V, Vs_V] = volume_fraction_PHREEQC(T,S,fn)
...
% composition and pressure defined by PHREEQC over the temperature range
% from the freezing point to the eutectic point. If T and S are both
% vectors, the brine volume fraction is calculated for each salinity and
% temperature where temperature varies across columns and salinity varies
% across rows.
%
% Syntax:
...
if P < pt
    ...
    if isscalar(S)
        Vs_V = interp1(T_pqo_mod,Vs_V,T,'linear');
    else
        [Smat_pqo,Tmat_pqo] = meshgrid(S,T_pqo_mod);
        Vs_V = interp2(Smat_pqo,Tmat_pqo,Vs_V,Smat,Tmat,'linear');
        if C(1)>0
            Vs_V(Tmat(:,1)>T_pqo(1),:) = 0;
        end
    end

    rho_ss = max(pqo.rho_ss)*1000;
    rho_i = ice_density(T(T<=T_pqo(end))+273.15,P);
    if ~isempty(rho_i)
        rho_i = repmat(rho_i,1,length(S));

        k_star = max(pqo.sm)/ms_b0;
        Vs_V(Tmat<=T_pqo(end)) = S*k_star./...
    ...
    F1 = interp1(T_pqo,F1,T,'linear');
    F2 = interp1(T_pqo,F2,T,'linear');
